        ...
        <div class="card-body">
            <form method="POST" action="{{ route('clients.update', $client->id) }}" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="file" class="form-control" name="client_image" id="client_image" value="{{$client->image}}">
                    @error('client_image')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                    <img class="mt-4" style="width: 40em;" id="img-preview" src="{{ asset('storage/'.$client->image) }}" alt="">
                </div>
